...
- **Audit Note:** A critical incident occurred where the Boss Agent added the entire `.agent/` folder to `.gitignore`. This was identified and corrected. The workflows were intentionally modified to _remove_ `// turbo` execution commands from critical steps to guarantee 100% manual Human CTO approval on all file writes and git merges.
