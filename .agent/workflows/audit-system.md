...
4. **Holistic Synthesis & Analysis (TCA Enforced):** Put all three pillars (Code, Docs, Rules) into context. Do they align? **CRITICAL:** Apply your `TEMPORAL_CONTEXT_AWARENESS` rule. Do not accuse the Orchestrator of hallucinating if their past logs simply reflect a frozen state before physical desyncs were discovered. Treat these as "Pending Remediation", not active lies.
